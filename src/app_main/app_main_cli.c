 ...
 * app_main_log_rx_callback(). When a full line is received (CR/LF),
 ...
 * app_main_log_puts/Write or ptxCommon_PrintF.
 */
...
#define APP_MAIN_CLI_CMD_COUNT   (sizeof(s_cmds) / sizeof(s_cmds[0]))
...
static volatile char     s_line[APP_MAIN_CLI_LINE_MAX + 1u];
 ...
 * and sets s_line_ready. Main-context app_main_cli_process() picks it up. */
static char              s_pending_line[APP_MAIN_CLI_LINE_MAX + 1u];
 ...
 * ####################################################################################################################
 */
static void app_main_cli_write (const char *s)
{
    app_main_log_puts(s);
}

static void app_main_cli_write_byte (uint8_t b)
{
    app_main_log_write(&b, 1u);
}

void app_main_cli_prompt (void)
{
    app_main_cli_write(APP_MAIN_CLI_NEWLINE APP_MAIN_CLI_PROMPT);
}

static int app_main_cli_streq_ci (const char *a, const char *b)
{
    while (*a && *b)
    {
        char ca = *a;
...
static void app_main_cli_cmd_version (const char *args)
{
    (void)args;
...
{
    (void)args;
    app_main_cli_print_menu();
}

static void app_main_cli_cmd_menu (const char *args)
{
    (void)args;
    app_main_cli_print_menu();
}

static void app_main_cli_cmd_reboot (const char *args)
{
    (void)args;
    app_main_cli_write("rebooting..." APP_MAIN_CLI_NEWLINE);
...
{
    { "help",    app_main_cli_cmd_help,    "show this menu"                 },
...
static void cli_dispatch (char *line)
{
    ...
    for (size_t i = 0u; i < APP_MAIN_CLI_CMD_COUNT; i++)
    {
        if (app_main_cli_streq_ci(line, s_cmds[i].name))
        {
    ...
    app_main_cli_write("unknown command: '");
    app_main_cli_write(line);
    app_main_cli_write("'  (type 'help')" APP_MAIN_CLI_NEWLINE);
}

static void app_main_cli_handle_byte (uint8_t b)
{
    ...
    if ((b == '\r') || (b == '\n'))
    {
        app_main_cli_write(APP_MAIN_CLI_NEWLINE);
        s_line[s_line_len] = '\0';
        if (s_line_len > 0u)
        {
            /* Dispatch the command directly in ISR context. All command handlers
             * only set flags or call app_main_log_write (which is ISR-safe), so
            ...
            app_main_cli_write(APP_MAIN_CLI_COLOR_KGRN APP_MAIN_CLI_PROMPT);
        }
        else
        {
    ...
    if (s_line_len < APP_MAIN_CLI_LINE_MAX)
    {
        s_line[s_line_len++] = (char)b;
        app_main_cli_write_byte(b); /* local echo */
    }
    else
    {
        /* Line full: beep. */
        app_main_cli_write_byte(0x07u);
    }
}

/*
 * ####################################################################################################################
 * ISR CALLBACK (registered with app_main_log_rx_callback)
 * ####################################################################################################################
 */
static void app_main_cli_rx_isr_callback (uint8_t byte)
{
    if (0u == s_initialized) { return; }
    app_main_cli_handle_byte(byte);
}
...
{
    app_main_cli_write(APP_MAIN_CLI_COLOR_KCYN);
    app_main_cli_write(APP_MAIN_CLI_NEWLINE);
    app_main_cli_write("=== PTX IoT Reader CLI ===" APP_MAIN_CLI_NEWLINE);

    for (size_t i = 0u; i < APP_MAIN_CLI_CMD_COUNT; i++)
    {
        /* Format: "  name        - help" with a simple fixed-width pad. */
        app_main_cli_write("  ");
        app_main_cli_write(s_cmds[i].name);
        size_t n = strlen(s_cmds[i].name);
        while (n < 10u) { app_main_cli_write_byte((uint8_t)' '); n++; }
        app_main_cli_write(" - ");
        app_main_cli_write(s_cmds[i].help);
        app_main_cli_write(APP_MAIN_CLI_NEWLINE);
    }

    app_main_cli_write(APP_MAIN_CLI_COLOR_KNRM);
}
...
void app_main_cli_init (void)
{
    ...
    app_main_log_rx_callback(app_main_cli_rx_isr_callback);
