 ...
 * supported — see rs_ndef_read.c for the flash-budget rationale.
 * Also provides RS_NDEF_BuildTextRecord() for constructing RTD-Text records.
 *
 * NO printing — returns rs_status_t only.
 */
...
static rs_status_t write_t4t_ndef(const uint8_t *ndef, uint16_t ndef_len)
{
    ...
    rs_nfc_ptx_ndef_close();
    return (ptxStatus_Success == ptx_st) ? RS_OK : RS_ERR_INTERNAL;
}
...
static rs_status_t write_t5t_ndef(const uint8_t *ndef, uint16_t ndef_len)
{
...
static rs_status_t write_t2t_ndef(const uint8_t *ndef, uint16_t ndef_len)
{
    ...
    if (ndef_len > 248u) { return RS_ERR_INVALID_CFG; }
    ...
    while (offset < tlv_len)
    {
        ...
        rx_len = RX_BUF_SIZE;
        rs_status_t st = rs_nfc_ptx_data_exchange(
            cmd, 6u, rx, &rx_len);
        if (RS_OK != st)
        {
                                         ...
                                         uint16_t ndef_len)
{
    switch (protocol)
    {
        ...
        default:
            return RS_ERR_INVALID_CFG;  /* protocol not supported */
    }
}
...
rs_status_t RS_NFCReader_EraseNDEF(rs_nfc_protocol_t protocol)
{
    ...
    return RS_NFCReader_WriteNDEF(protocol, NULL, 0u);
}
