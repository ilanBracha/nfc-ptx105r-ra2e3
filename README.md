...
![FPB-RA2E3 + PTX105R connected to PMOD1](README_NFC_assets/FPB-RA2E3_PTX105R.png)
...
![PTX105R QC PMOD Board](README_NFC_assets/PTX105R-DB-RB.png)
...
![PTX105R QC PMOD Board](README_NFC_assets/PTX105R-Schematic.png)
...
- **e² studio**: [e² studio Integrated Development Environment User's Manual: Getting Started Guide](https://tool-support.renesas.com/e2studio/vscode/docs/creating-and-building-project.html#using-renesas-ra) — See **CHAPTER 3: PROJECT GENERATION**
- **Renesas Smart Configurator (RASC)**: [RA Smart Configurator | Renesas](https://www.renesas.com/en/software-tool/ra-smart-configurator#additional_details) - See **Additional Details**
...
| ROM / RAM | 65,536 B (64 KB) / 16,384 B (16 KB) |
...
![Figure 4a — FSP Stacks tab: HAL/Common top-level view](README_NFC_assets/FSP_Config_stack1.png)

*Figure 4a — FSP Stacks tab: HAL/Common top-level view*



![Figure 4b — FSP Stacks tab: g_nfc_reader_ptx0 full stack expanded](README_NFC_assets/FSP_Config_stack2.png)
...
![Figure 5 — Pin Assignments](README_NFC_assets/FSP_Config_PIN.png)
...
### Step 2: Import the Project into e² studio
...
3. Click **Browse** → navigate to the `nfc-ptx105r-ra2e3` project folder. The wizard detects **Ra2e1_PtxIotReader** automatically. Click **Finish**.
...
**** Incremental Build of configuration Debug ****
  ...
  62636     160   15157   77953   13081 Ra2e1_PtxIotReader.elf
Build Finished. 0 errors, 0 warnings.
```

Output files are written to `Debug/`: `Ra2e1_PtxIotReader.elf` · `.srec` · `.map`

### Step 5: Flash & Run

1. Confirm the micro-USB cable is connected to J10 and the board is powered.
2. **Run → Debug Configurations...** → double-click **Renesas GDB Hardware Debugging → Ra2e1_PtxIotReader Debug_Flat**.
...
└── Ra2e1_PtxIotReader Debug_Flat.launch  ← Pre-configured J-Link debug launch
```

> **Note:** There is **no `rs_ndef_write.c`** — this release is read-only. Never manually edit files in `ra_gen/` or `ra_cfg/`. These are regenerated by the FSP tooling whenever `configuration.xml` is saved.
...
`rs_nfc_reader_cfg_t` is the main input to RS. The application layer fills it in `app_nfc_reader_init()` before calling `rs_nfc_reader_Read()`.
...
> **Note:** There are **no** `WriteNDEF` / `EraseNDEF` / `BuildTextRecord` / `DataExchange` / `RawExchange` public functions — those are not part of this read-only release.
...
- The result struct is reused by RS, so copy any data you need after the callback returns.
...
- This release does not write to tags; `result->writeable` is informational only.
...
For a quick start: import the project into e² studio, build with the **Debug** configuration, flash via J-Link, and open a serial terminal at 115200 baud.
